...
if [[ "$@" == "" ]]; then
    pyinstaller --clean -y --dist ./dist/ --workpath /tmp scrambled-strings.py
    chown -R --reference=. ./dist
	../dist/scrambled-strings --dictionary ../data/dict.txt --input ../data/input.txt --loglevel DEBUG >> ../data/docker_output.txt
else
    sh -c "$@"
fi # [[ "$@" == "" ]]
